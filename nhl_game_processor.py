...
class GameProcessor:
    ...
    def _get_first_goal_team(game_id):
        ...
        try:
            ...
            if response.ok:
                ...
                first_goal = next(
                    (play for play in plays if play.get('typeDescKey') == 'goal'),
                    None
                )
                if first_goal:
    ...
    def process_game(game_details, team_code, game_id=None):
        ...
        try:
            ...
            opponent_players = player_stats.get('awayTeam' if is_home else 'homeTeam', {})

            # Process special teams stats
            for player_type in ['forwards', 'defense']:
                for player in team_players.get(player_type, []):
                    game_stats['powerplay_goals'] += int(player.get('powerPlayGoals', 0))
            ...
            if starter_goalie:
                es_shots = GameProcessor._parse_shot_string(starter_goalie.get('evenStrengthShotsAgainst', '0/0'))
                pp_shots = GameProcessor._parse_shot_string(starter_goalie.get('powerPlayShotsAgainst', '0/0'))
                sh_shots = GameProcessor._parse_shot_string(starter_goalie.get('shorthandedShotsAgainst', '0/0'))
            ...
            our_starter = next((g for g in our_goalies if g.get('starter', False)), None)
            if our_starter:
                es_shots = GameProcessor._parse_shot_string(our_starter.get('evenStrengthShotsAgainst', '0/0'))
                pp_shots = GameProcessor._parse_shot_string(our_starter.get('powerPlayShotsAgainst', '0/0'))
                sh_shots = GameProcessor._parse_shot_string(our_starter.get('shorthandedShotsAgainst', '0/0'))
                ...
                game_stats['penalty_kill_successes'] = game_stats['times_shorthanded'] - opponent_pp_goals
            ...
            if game_stats['goals_for'] > game_stats['goals_against']:
                ...
                period_desc = game_details.get('periodDescriptor', {}).get('periodType', '')
                if 'OT' in game_state or 'SO' in game_state or 'OT' in period_desc or 'SO' in period_desc:
                    game_stats['otl'] = 1
            ...
            logging.error(f"Error processing game for {team_code}: {str(e)}", exc_info=True)
            return game_stats
    ...
    def aggregate_stats(game_stats_list):
        ...
        for i, game in enumerate(game_stats_list, 1):
            ...
            for key in total_stats:
                ...
                elif key != 'games_played':  # games_played is calculated from list length
        ...
        if games > 0:
            ...
            if total_stats['powerplay_opportunities'] > 0:
                total_stats['powerplay_percentage'] = (
                    ...
                    total_stats['powerplay_opportunities'] * 100
                )
            else:
            ...
            if total_stats['times_shorthanded'] > 0:
                total_stats['penalty_kill_percentage'] = (
                    ...
                    total_stats['times_shorthanded'] * 100
                )
            else:
